...
{
	return *((char *)(&value->flags) + VALUE_STATIC_OFFSET) == lb_static;
}

inline char value_is_const(const value_t *value)
{
	return *((char *)(&value->flags) + VALUE_CONST_OFFSET) == lb_const;
}

inline unsigned char *value_manager_flags(const value_t *value)
{
	return ((char *)(&value->flags) + VALUE_MANAGER_FLAGS_OFFSET);
}

inline unsigned char value_typeof(const value_t *value)
{
	return *((char *)(&value->flags) + VALUE_TYPE_OFFSET);
}

inline void value_set_type(const value_t *value, unsigned char type)
{
	*((char *)(&value->flags) + VALUE_TYPE_OFFSET) = type;
}
...
inline unsigned char field_typeof(const field_t *field)
{
	return value_typeof((const value_t *)field);
}


inline size_t sizeof_type(unsigned char type)
{
	switch (type)
	{
	case lb_char:
